...
public class ItemRecommendation {
    ...
    private static final int DEFAULT_TOTAL_RECOMMENDATION_LIMIT = 20;//static+final便于编译器处理
    ...
    private List<Item> recommendByTopGames(ItemType type, List<Game> topGames) throws RecommendationException{
        ...
        outerloop://名字无所谓 : 代表break的标识
        for (Game game: topGames){
            List<Item> items = client.searchByType(game.getId(),type, DEFAULT_PER_GAME_RECOMMENDATION_LIMIT);
            for (Item item: items) {
                if (recommendations.size() == DEFAULT_PER_GAME_RECOMMENDATION_LIMIT) break outerloop;
    ...
    private List<Item> recommendByFavoriteHistory(
        ...
        //--> 翻译翻译:List<gameId> -> Map(gameId, count)
        //parallelStream: 多核时并行操作->比单核for loop快
        //identity? 类似callback return本身，可以用于对input初步加工
